...
SparccToNet <- function(mbSetObj=NULL, corr.net.name, networkType="static", 
                        netLayout="kk", netTextSize){
  ...
  if(.on.public.web){
    ...
    netData <- list(nodes=nodes, abund=propList, abundColor=abundColor, edges=edge.mat, 
                    taxa=taxa, taxaNodes = taxNodes)#, modules = modules, taxaNodes = list(tax=colVecNms,colors=topo.colsb));
    sink(corr.net.name);
    cat(toJSON(netData));
    sink();
  }
...
CoreMicrobeAnalysis<-function(mbSetObj, imgName, preval, detection, taxrank,
                              ...
                              format="png", dpi=72, width=NA, interactive = T){
...
PerformBetaDiversity <- function(mbSetObj, plotNm, ordmeth, distName, colopt, metadata, 
  ...
  if(interactive){
    ...
    p <- plotly::ggplotly(box)
    p <- p %>% layout(xaxis = ax, yaxis = ax)
    htmlwidgets::saveWidget(p, "beta_diversity_interactive.html")
  }
...
PlotTaxaAundanceBar<-function(mbSetObj, barplotName, taxalvl, facet, facet2, imgOpt, 
  ...
  if(interactive){
    ...
    p <- plotly::ggplotly(box)
    p <- p %>% layout(xaxis = ax, yaxis = ax)
    htmlwidgets::saveWidget(p, "stacked_bar_interactive.html")
  }
...
PerformCategoryComp <- function(mbSetObj, taxaLvl, method, distnm, variable,
  ...
  if(method=="adonis"){
    ...
    if(covariates){
      ...
      res2 <- adonis(formula = f, data = sampledf)
